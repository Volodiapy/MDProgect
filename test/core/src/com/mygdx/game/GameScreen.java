...
public class GameScreen implements Screen{
	...
	public void endWin(){
		try {
			...
			if(game.stage < 8 & game.stage % 2 == 1){
				game.enemywhatspells[(game.stage+1)/2 + 1] = String.valueOf(9+(game.stage + 1)/2);
	...
	public GameScreen(final Drop gam, MainMenuScreen mainMS) {
		this.game = gam;
		this.mainMenuScreen = mainMS;
		...
		if (game.stage % 2 == 1 & game.stage < 8){
...
//			fonsprite.end();
			...
			if(Gdx.input.isTouched()) {
				...
				if (buff1.contains(touchPos.x, touchPos.y)) {
					...
					if (game.stage % 2 == 1 & game.stage < 8){
						game.whatspells[(game.stage+1)/2 + 1] = String.valueOf(3 + (game.stage + 1)/2);
					}
					else {
						game.coin += 1;
					}
	...
	public void dispose () {
		fonsprite.dispose();
		healthfont.dispose();
		modattackfont.dispose();
		moddeffont.dispose();
		modcontrolfont.dispose();
		damageview.dispose();
		enemydamageview.dispose();
